...
class BillingWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.repo_cli = Repository("clientes.txt")
        self.repo_prod = Repository("productos.txt")
        self.repo_fact = Repository("facturas.txt")
        self.repo_detalle = Repository("detalle_factura.txt")
        
        self.clientes = self.repo_cli.load_all(Cliente.from_string)
        self.productos = self.repo_prod.load_all(Producto.from_string)
    ...
    def save_invoice(self):
        ...
        # Get next ID
        existing = self.repo_fact.load_all(Factura.from_string)
        new_id = 1
        if existing: new_id = max([f.codigo for f in existing]) + 1
        
        total = sum([i['qty'] * i['price'] for i in self.cart])
        date_str = datetime.now().strftime("%d/%m/%Y")
        
        # Create Invoice
        fact = Factura(new_id, cli_id, date_str, total)
        self.repo_fact.append(fact)
        
        
        # Save Details and Update Stock
        for item in self.cart:
            det = DetalleFactura(new_id, item['prod'].codigo, item['qty'], item['price'])
            self.repo_detalle.append(det)
            
            item['prod'].cantidad -= item['qty']
            
        self.repo_prod.save_all(self.productos)
        
        # Generate PDF
        try:
            from src.reports import PDFGenerator
            gen = PDFGenerator()
            fpath = gen.generate_invoice_pdf(fact, next(c for c in self.clientes if c.codigo == cli_id), self.cart)
            msg = f"Factura #{new_id} generada. Inventario actualizado.\nreporte guardado en {fpath}"
        except ImportError:
            msg = f"Factura #{new_id} generada. (Modulo fpdf no instalado para PDF)"
        except Exception as e:
            msg = f"Factura #{new_id} generada. Error creando PDF: {str(e)}"
