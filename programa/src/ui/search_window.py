...
class SearchTransactionsWindow(tk.Toplevel):
    def __init__(self, parent, mode="INVOICE"): # mode: INVOICE or ORDER
        ...
        self.repo_main = Repository("facturas.txt" if mode == "INVOICE" else "ordenes.txt")
        self.repo_det = Repository("detalle_factura.txt" if mode == "INVOICE" else "detalle_orden.txt")
        self.repo_prod = Repository("productos.txt")
    ...
    def perform_search(self):
        ...
        items = self.repo_main.load_all(cls.from_string)
    ...
    def on_select(self, event):
        ...
        # Load Details
        # This is inefficient scanning full file but strictly adhering to text file repo
        from src.models import DetalleOrden
        cls_det = DetalleFactura if self.mode == "INVOICE" else DetalleOrden
        
        all_dets = self.repo_det.load_all(cls_det.from_string)
    ...
    def anulate_transaction(self):
        ...
        # Logic
        # 1. Get Details
        cls_det = DetalleFactura if self.mode == "INVOICE" else type('DetalleOrden', (), {}) # HACK: Need proper class
        from src.models import DetalleOrden
        if self.mode != "INVOICE": cls_det = DetalleOrden
        ...
        # 2. Update Stock
        stock_repo = Repository("productos.txt")
        prods = stock_repo.load_all(Producto.from_string)
        ...
        stock_repo.save_all(prods)
        
        # 3. Delete Transaction? Or Mark as Anulated?
        # Requirements imply "Se podrá anular". Usually soft delete or status update.
        # But my models don't have Status field. 
        # "Borrar" vs "Anular".
        # Let's DELETE the transaction to keep it simple with current models, OR we need to add a Status field to models.
        # Adding status field breaks existing file format if not careful.
        # Let's Delete row + Delete details.
        
        main_cls = Factura if self.mode == "INVOICE" else OrdenCompra
        all_trans = self.repo_main.load_all(main_cls.from_string)
        all_trans = [t for t in all_trans if t.codigo != trans_id]
        self.repo_main.save_all(all_trans)
        
        # Cleanup details
        remaining_dets = [d for d in all_dets if getattr(d, attr_key) != trans_id]
        self.repo_det.save_all(remaining_dets)
