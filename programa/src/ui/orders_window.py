...
class OrdersWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.repo_prov = Repository("proveedores.txt")
        self.repo_prod = Repository("productos.txt")
        self.repo_orden = Repository("ordenes.txt")
        self.repo_detalle = Repository("detalle_orden.txt")
        
        self.proveedores = self.repo_prov.load_all(Proveedor.from_string)
    ...
    def save_order(self):
        ...
        # Get next ID
        existing = self.repo_orden.load_all(OrdenCompra.from_string)
        new_id = 1
        if existing: new_id = max([o.codigo for o in existing]) + 1
        
        total = sum([i['qty'] * i['cost'] for i in self.cart])
        date_str = datetime.now().strftime("%d/%m/%Y")
        
        # Create Order
        orden = OrdenCompra(new_id, prov_id, date_str, total)
        self.repo_orden.append(orden)
        ...
        # Reflush Products with new stock
        self.repo_prod.save_all(self.productos)
        
        messagebox.showinfo("Éxito", f"Orden #{new_id} guardada. Inventario actualizado.")
        self.destroy()
